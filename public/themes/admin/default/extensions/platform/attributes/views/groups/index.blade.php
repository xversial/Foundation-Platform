...
{{-- Page title --}}
@section('title')
{{{ trans('platform/attributes::groups/general.title') }}} ::
@parent
@stop
...
<div class="row">
	...
	<div class="col-md-12">
		...
		<div class="page-header">
				...
				<form method="post" action="" accept-charset="utf-8" data-search data-grid="main" class="form-inline" role="form">
					...
					<div class="form-group">
						<select class="form-control" name="column">
							...
							<option value="name">{{{ trans('platform/attributes::groups/table.name') }}}</option>
							<option value="created_at">{{{ trans('platform/attributes::groups/table.created_at') }}}</option>
						</select>
					</div>
			...
			<h1>{{{ trans('platform/attributes::groups/general.title') }}}</h1>
		...
		<div class="clearfix">
			...
			<div class="pull-right">
				<a class="btn btn-warning tip" href="{{ URL::toAdmin('attributes/groups/create') }}" title="{{{ trans('button.create') }}}"><i class="icon-plus"></i></a></li>
			</div>
		...
		<table data-source="{{ URL::toAdmin('attributes/groups/grid') }}" data-grid="main" class="data-grid table table-striped table-bordered table-hover">
			<thead>
				<tr>
					<th data-sort="name" data-grid="main" class="col-md-4 sortable">{{{ trans('platform/attributes::groups/table.name') }}}</th>
					<th data-sort="created_at" data-grid="main" class="col-md-3 sortable">{{{ trans('platform/attributes::groups/table.created_at') }}}</th>
			...
			<tbody>
				<tr data-template style="display: none;">
					...
					<td>[[ created_at ]]</td>
					<td>
						<a class="btn btn-danger tip" data-toggle="modal" data-target="modal-confirm" href="{{ URL::toAdmin('attributes/groups/delete/[[ id ]]') }}" title="{{{ trans('button.delete') }}}"><i class="icon-trash"></i></a>

						<a class="btn btn-primary tip" href="{{ URL::toAdmin('attributes/groups/edit/[[ id ]]') }}" title="{{{ trans('button.edit') }}}"><i class="icon-edit"></i></a>
					</td>
				</tr>
