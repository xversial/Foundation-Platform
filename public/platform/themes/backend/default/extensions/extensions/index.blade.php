                    ...
                    <td>
                        @if ( Platform::extensions_manager()->is_installed(array_get($extension, 'info.slug')) )
                            @if ( Platform::extensions_manager()->can_uninstall(array_get($extension, 'info.slug')) )
                                <a class="btn" href="{{ URL::to_admin('extensions/uninstall/' . array_get($extension, 'info.slug')) }}">{{ Lang::line('extensions::button.uninstall') }}</a>
                            @else
                                <a class="btn disabled">{{ Lang::line('extensions::button.uninstall') }}</a>
                            @endif
                            ...
                            @if ( Platform::extensions_manager()->is_enabled(array_get($extension, 'info.slug')) )
                                @if ( Platform::extensions_manager()->can_disable(array_get($extension, 'info.slug')) )
                                    <a class="btn" href="{{ URL::to_admin('extensions/disable/' . array_get($extension, 'info.slug')) }}">{{ Lang::line('extensions::button.disable') }}</a>
                                @else
                                    <a class="btn disabled">{{ Lang::line('extensions::button.disable') }}</a>
                                @endif
                            @else
                                @if ( Platform::extensions_manager()->can_enable(array_get($extension, 'info.slug')) )
                                    <a class="btn" href="{{ URL::to_admin('extensions/enable/' . array_get($extension, 'info.slug')) }}">{{ Lang::line('extensions::button.enable') }}</a>
                                @else
                                    <a class="btn disabled">{{ Lang::line('extensions::button.enable') }}</a>
                                @endif
                            @endif
                        @else
                            @if ( Platform::extensions_manager()->can_install(array_get($extension, 'info.slug')) )
                                <a class="btn" href="{{ URL::to_admin('extensions/install/' . array_get($extension, 'info.slug')) }}">{{ Lang::line('extensions::button.install') }}</a>
                            @else
                                <a class="btn disabled">{{ Lang::line('extensions::button.install') }}</a>
                            @endif
                        @endif
                    </td>
                </tr>
